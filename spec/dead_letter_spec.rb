...
RSpec.describe ::Pistonqueue::DlqConsumer, type: :reactor do
  describe "#redis_stream" do
    ...
    it "successfully received data from redis stream, and there was no problem processing consumer dead letter" do
      ...
      Async do |task|
        ...
        dead_letter_task = task.async do
          dead_letter.perform(topic: topic, task_type: :io_bound_light, group: group_name, consumer: consumer_name, is_stop: true) do |data|
    ...
    it 'successfully received data from redis stream, but there was a problem processing consumer dead letter' do
      ...
      Async do |task|
        ...
        dead_letter_task2 = task.async do
          dead_letter.perform(topic: topic, task_type: :io_bound_light, is_archive: true, group: group_name, consumer: consumer_name, is_stop: true) do |data|
            original_data = data[:original_data]
            expect(original_data["order_id"]).to eq(order_id)
            expect(original_data["total_payment"]).to eq(total_payment)
    ...
    it 'successfully received data from redis stream, but failed to process because the group does not exist' do
      ...
      Async do |task|
        dead_letter_task = task.async do
          dead_letter.perform(topic: topic, task_type: :io_bound_light, group: group_name, is_stop: true) do |data| end
    ...
    it 'successfully received data from redis stream, but failed because the is_archive parameter is invalid' do
      order_id = "ORD-#{SecureRandom.uuid}"
      total_payment = rand(10000..99999)

      Async do |task|
        dead_letter_task = task.async do
          dead_letter.perform(topic: topic, task_type: :io_bound_light, is_archive: 'haha', group: group_name, is_stop: true) do |data| end
        rescue ArgumentError => e
          expect(e.message).to eq("The 'is_archive' parameter must be a boolean.")
        end
